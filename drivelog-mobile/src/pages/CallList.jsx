...
function CreateCallModal({ onClose, onCreated }) {
  ...
  useEffect(() => {
    if (custSearch.length >= 1) {
      fetchCustomers({ q: custSearch, limit: 10 }).then(r => { setCustomers(r.data || []); setShowCustList(true); }).catch(() => {});
    } else { setCustomers([]); setShowCustList(false); }
  }, [custSearch]);

  useEffect(() => {
    if (partSearch.length >= 1) {
      fetchPartners({ q: partSearch, limit: 10 }).then(r => { setPartners(r.data || []); setShowPartList(true); }).catch(() => {});
    } else { setPartners([]); setShowPartList(false); }
  }, [partSearch]);
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 210, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: '20px 20px 0 0', padding: '24px 20px 32px', width: '100%', maxWidth: 420, maxHeight: '85vh', overflowY: 'auto' }}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <input value={form.start_address} onChange={e => setForm(f => ({ ...f, start_address: e.target.value }))} placeholder="출발지 주소" style={is} />
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <input value={form.end_address} onChange={e => setForm(f => ({ ...f, end_address: e.target.value }))} placeholder="도착지 주소 (미정 가능)" style={is} />
...
export default function CallList({ user }) {
  ...
  const handleCancelCall = async (callId) => {
    ...
    try {
      await cancelCall(callId, { cancel_reason: '관리자 취소' });
      load();
    } catch (err) { alert(err.response?.data?.error || '취소 실패'); }
  };
    ...
    <div className="fade-in" style={{ minHeight: '100vh', background: '#f7f8fc' }}>
      ...
      <div style={{ padding: '16px 16px' }}>
        {loading ? (
          ...
          <>
            {/* RIDER: 내 진행 콜 / SUPER_ADMIN: 전체 진행 콜 */}
            ...
            {waitingCalls.length === 0 ? (
              <div style={{ background: 'white', borderRadius: 16, padding: '40px 20px', textAlign: 'center', border: '1px solid #f1f5f9' }}>
                ...
                <div style={{ fontSize: 12, color: '#cbd5e1', marginTop: 4 }}>
                  {isSuperAdmin ? '위 "콜 생성" 버튼으로 새 콜을 만드세요' : '30초마다 자동으로 새 콜을 확인합니다'}
                </div>
              </div>
            ) : (
                  ...
                  {isSuperAdmin ? (
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button onClick={() => handleAccept(call.call_id)} disabled={accepting === call.call_id}
                        style={{ flex: 2, padding: 12, borderRadius: 12, border: 'none', background: accepting === call.call_id ? '#94a3b8' : '#d97706', color: 'white', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
                        {accepting === call.call_id ? '수락 중...' : '📞 직접 수락'}
                      </button>
                      <button onClick={() => handleCancelCall(call.call_id)}
                        style={{ flex: 1, padding: 12, borderRadius: 12, border: '1px solid #fecaca', background: '#fef2f2', color: '#dc2626', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>취소</button>
                    </div>
                  ) : (
                    <button onClick={() => handleAccept(call.call_id)} disabled={accepting === call.call_id}
