...
export default function KakaoMiniMap({ startLat, startLng, endLat, endLng, height = 250 }) {
  ...
  useEffect(() => {
    ...
    const map = new kakao.maps.StaticMap(mapRef.current, {
      center, level,
      marker: false
    });

    // 마커는 StaticMap에서 직접 지원하지 않으므로 overlay로 대체
    // StaticMap은 마커 옵션이 있음
    // 재생성: marker 포함
    mapRef.current.innerHTML = '';

    const markers = [];
    if (hasStart) {
      markers.push({ position: new kakao.maps.LatLng(startLat, startLng), text: '출발' });
    }
    if (hasEnd) {
      markers.push({ position: new kakao.maps.LatLng(endLat, endLng), text: '도착' });
    }

    new kakao.maps.StaticMap(mapRef.current, {
      center, level,
      marker: markers
    });
