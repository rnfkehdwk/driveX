...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    const [rows] = await pool.execute(`SELECT b.*, c.company_name, c.company_code FROM app_billing b JOIN companies c ON b.company_id = c.company_id ${where} ORDER BY b.billing_period DESC`, params);
    res.json({ data: rows });
...
router.post('/generate', authenticate, authorize('MASTER'), async (req, res) => {
  ...
  try {
    ...
    const [stats] = await conn.execute(`SELECT r.company_id, c.company_name, COUNT(*) AS total_rides FROM rides r JOIN companies c ON r.company_id = c.company_id WHERE c.status = 'ACTIVE' AND DATE_FORMAT(r.started_at, '%Y-%m') = ? AND r.status != 'CANCELLED' GROUP BY r.company_id, c.company_name`, [billing_period]);
    const created = [];
    for (const stat of stats) {
      const [existing] = await conn.execute('SELECT billing_id FROM app_billing WHERE company_id = ? AND billing_period = ?', [stat.company_id, billing_period]);
      if (existing.length > 0) continue;
      const billingAmount = 50000;
      const [result] = await conn.execute(`INSERT INTO app_billing (company_id, billing_period, total_rides, billing_amount, status) VALUES (?, ?, ?, ?, 'INVOICED')`, [stat.company_id, billing_period, stat.total_rides, billingAmount]);
  ...
  } catch (err) { await conn.rollback(); res.status(500).json({ error: '사용료 생성에 실패했습니다.' }); }
