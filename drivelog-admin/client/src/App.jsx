  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      {user && (
        <>
          <header style={{
            background: 'white', borderBottom: '1px solid #e2e8f0', padding: '0 16px',
            ...
            <button onClick={() => setSidebarOpen(true)} style={{
              ...
              justifyContent: 'center', gap: 5, marginRight: 12, flexShrink: 0,
            }}>
          ...
          <div style={{
            position: 'fixed', top: 0, left: 0, bottom: 0, width: 280, background: 'white', zIndex: 101,
            transform: sidebarOpen ? 'translateX(0)' : 'translateX(-100%)',
            transition: 'transform 0.25s ease-in-out', display: 'flex', flexDirection: 'column',
                  ...
                  {group.items.map(item => {
                    ...
                    return (
                      <NavLink key={item.path} to={item.path} end={item.path === '/'} onClick={() => setSidebarOpen(false)}
                        style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 20px', textDecoration: 'none',
                          background: isActive ? '#eff6ff' : 'transparent', borderRight: isActive ? '3px solid #2563eb' : '3px solid transparent' }}>
            ...
            <div style={{ padding: '12px 16px', borderTop: '1px solid #f1f5f9' }}>
              <button onClick={() => { setSidebarOpen(false); handleLogout(); }} style={{
                width: '100%', padding: '12px 0', borderRadius: 10, border: '1px solid #fee2e2',
                background: '#fef2f2', fontSize: 14, fontWeight: 600, color: '#dc2626', cursor: 'pointer', fontFamily: 'inherit',
              }}>로그아웃</button>
            </div>
          </div>
        </>
