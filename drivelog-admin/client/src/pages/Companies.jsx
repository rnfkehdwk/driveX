...
export default function Companies() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      if (editing) { const { company_code, ...body } = form; await updateCompany(editing.company_id, body); }
  ...
  const headers = [
    { key: null, label: '업체코드' }, { key: 'company_name', label: '업체명' }, { key: 'ceo_name', label: '대표' },
    { key: null, label: '연락처' }, { key: null, label: '기사수' }, { key: null, label: '월운행' },
    { key: null, label: '상태' }, { key: null, label: '등록일' }, { key: null, label: '관리' },
  ];
