...
export default function PartnerManage() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await updatePartner(editing.partner_id, { name: form.name, phone: form.phone, address: form.address, contact_person: form.contact_person, memo: form.memo });
      } else {
  ...
  const headers = isMaster
    ? ['소속업체', '업체명', '연락처', '주소', '담당자', '메모', '상태', '관리']
    : ['업체명', '연락처', '주소', '담당자', '메모', '상태', '관리'];

  return (
    <div className="fade-in">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <span style={{ fontSize: 13, color: '#94a3b8' }}>등록 업체 {list.length}개</span>
        <button onClick={openNew} style={{ padding: '8px 18px', borderRadius: 8, background: '#7c3aed', color: 'white', border: 'none', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
          + 제휴업체 등록
        </button>
      </div>
      ...
      <div style={{ background: 'white', borderRadius: 14, border: '1px solid #f1f5f9', overflow: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          ...
          <tbody>
            {list.map(p => (
              <tr key={p.partner_id} style={{ borderBottom: '1px solid #f8fafc' }}>
                ...
                <td style={{ padding: '11px 12px' }}>
                  <span style={{ padding: '2px 8px', borderRadius: 4, fontSize: 11, fontWeight: 600, background: p.status === 'ACTIVE' ? '#f0fdf4' : '#fef2f2', color: p.status === 'ACTIVE' ? '#16a34a' : '#dc2626' }}>{p.status}</span>
                </td>
                <td style={{ padding: '11px 12px' }}>
            ...
            {list.length === 0 && <tr><td colSpan={headers.length} style={{ padding: 40, textAlign: 'center', color: '#94a3b8' }}>제휴업체가 없습니다.</td></tr>}
          </tbody>
        </table>
      </div>
      ...
      {modal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }} onClick={() => setModal(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: 16, padding: 28, width: 420, boxShadow: '0 16px 48px rgba(0,0,0,0.15)' }}>
            ...
            {[
              { k: 'name', label: '업체명 *', ph: '녹원갈비' },
              { k: 'phone', label: '연락처', ph: '033-671-2325' },
              { k: 'address', label: '주소', ph: '강원도 양양군...' },
              { k: 'contact_person', label: '담당자', ph: '김담당' },
              { k: 'memo', label: '메모', ph: '참고사항' },
            ].map(f => (
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 20 }}>
              ...
              <button onClick={handleSave} disabled={saving} style={{ flex: 2, padding: '12px 0', borderRadius: 10, border: 'none', background: '#7c3aed', color: 'white', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
