...
router.post('/login', async (req, res) => {
  ...
  try {
    let sql = `SELECT u.*, c.company_code, c.company_name, c.status AS company_status,
               c.trial_expires_at, c.license_expires, c.license_type, c.plan_id,
               p.plan_name
               FROM users u
    ...
    if (user.role !== 'MASTER' && user.company_status) {
      if (user.company_status === 'PENDING') return res.status(403).json({ error: '업체 승인 대기 중입니다. 관리자 승인 후 이용 가능합니다.' });
      if (user.company_status === 'SUSPENDED') return res.status(403).json({ error: '업체가 정지 상태입니다. 관리자에게 문의하세요.' });
      if (user.company_status === 'TRIAL' && user.trial_expires_at && new Date(user.trial_expires_at) < new Date()) return res.status(403).json({ error: '무료 체험 기간이 만료되었습니다. 유료 전환을 위해 관리자에게 문의하세요.' });
      if (!['ACTIVE', 'TRIAL'].includes(user.company_status)) return res.status(403).json({ error: `업체 상태: ${user.company_status}. 관리자에게 문의하세요.` });
    }
    ...
    let licenseExpiresDate = null;
    if (user.role !== 'MASTER' && user.license_expires) {
      licenseExpiresDate = user.license_expires;
      const expDate = new Date(user.license_expires);
      expDate.setHours(23, 59, 59, 999);
      if (now > expDate) licenseExpired = true;
    }
    ...
    const responseUser = {
      ...
      // 사용료 관련 정보
      license_expires: licenseExpiresDate,
  ...
  try {
    const [rows] = await pool.execute(
      `SELECT u.user_id, u.company_id, u.role, u.name, u.phone, u.email,
              ...
              c.company_code, c.company_name, c.status AS company_status,
              c.trial_expires_at, c.license_expires, c.license_type, c.plan_id,
              p.plan_name
       FROM users u
       LEFT JOIN companies c ON u.company_id = c.company_id
    ...
    if (r.role !== 'MASTER' && r.license_expires) {
      const exp = new Date(r.license_expires); exp.setHours(23,59,59,999);
      r.license_expired = now > exp;
    } else { r.license_expired = false; }
