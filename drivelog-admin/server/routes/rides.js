...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    // 멀티테넌트: RIDER는 본인만, SUPER_ADMIN/MASTER는 업체 전체
...
router.get('/:id', authenticate, async (req, res) => {
  try {
    ...
    // 멀티테넌트 검증
    const ride = rows[0];
    if (req.user.role !== 'MASTER' && ride.company_id !== req.user.company_id) {
      return res.status(403).json({ error: '접근 권한이 없습니다.' });
    }
    if (req.user.role === 'RIDER' && ride.rider_id !== req.user.user_id) {
      return res.status(403).json({ error: '본인의 운행 기록만 조회할 수 있습니다.' });
    }
...
router.post('/', authenticate, authorize('RIDER', 'SUPER_ADMIN'), async (req, res) => {
  ...
  try {
    ...
    const {
      ...
      started_at, ended_at,
      total_fare, cash_amount, mileage_used, payment_method,
      rider_memo,
    } = req.body;
    ...
    if (total_fare) {
      const [policies] = await conn.execute(
        `SELECT mileage_earn_pct FROM fare_policies 
         WHERE company_id = ? AND is_active = TRUE AND effective_from <= CURDATE()
         ORDER BY effective_from DESC LIMIT 1`,
        [req.user.company_id]
      );
      if (policies.length > 0) {
        mileage_earned = Math.floor(total_fare * policies[0].mileage_earn_pct / 100);
      }
    }
    ...
    const [result] = await conn.execute(
      `INSERT INTO rides (
        ...
        status, start_address, start_detail, start_lat, start_lng,
        end_address, end_detail, end_lat, end_lng,
        started_at, ended_at,
        total_fare, cash_amount, mileage_used, mileage_earned,
        final_amount, payment_method, rider_memo
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        req.user.company_id, req.user.user_id, pickup_rider_id || null,
        customer_id || null, partner_id || null,
        ended_at ? 'COMPLETED' : 'STARTED',
        ...
        total_fare || null, cash_amount || null, mileage_used || 0, mileage_earned,
        total_fare ? (total_fare - (mileage_used || 0)) : null,
        payment_method || 'CASH', rider_memo || null,
      ]
    );
    ...
    if (start_lat && start_lng) {
      await conn.execute(
        `INSERT INTO manual_gps_points (ride_id, point_type, latitude, longitude, address, input_method, recorded_at)
         VALUES (?, 'START', ?, ?, ?, 'CURRENT_LOCATION', ?)`,
        [rideId, start_lat, start_lng, start_address, started_at]
      );
    }
    if (end_lat && end_lng && ended_at) {
      await conn.execute(
        `INSERT INTO manual_gps_points (ride_id, point_type, latitude, longitude, address, input_method, recorded_at)
         VALUES (?, 'END', ?, ?, ?, 'CURRENT_LOCATION', ?)`,
        [rideId, end_lat, end_lng, end_address, ended_at]
      );
    }
    ...
    if (mileage_earned > 0 && customer_id) {
      ...
      await conn.execute(
        `INSERT INTO customer_mileage (customer_id, company_id, type, amount, balance_after, description, ride_id, processed_by)
         VALUES (?, ?, 'EARN', ?, ?, '운행 마일리지 적립', ?, ?)`,
        [customer_id, req.user.company_id, mileage_earned, balanceAfter, rideId, req.user.user_id]
      );
    ...
    writeAuditLog({
      ...
      action: 'RIDE_CREATE', target_table: 'rides', target_id: rideId,
      ip_address: req.ip,
    });
    ...
    res.status(500).json({ error: '운행일지 저장에 실패했습니다.' });
  } finally {
    conn.release();
  }
});

// PUT /api/rides/:id - 운행 수정
router.put('/:id', authenticate, async (req, res) => {
  try {
    const fields = req.body;
    const allowed = [
      'end_address', 'end_detail', 'end_lat', 'end_lng', 'ended_at',
      'total_fare', 'cash_amount', 'mileage_used', 'payment_method',
      'rider_memo', 'admin_memo', 'status', 'partner_id', 'customer_id',
    ];
    ...
    writeAuditLog({
      company_id: req.user.company_id, user_id: req.user.user_id,
      action: 'RIDE_UPDATE', target_table: 'rides', target_id: parseInt(req.params.id),
      detail: { updated_fields: Object.keys(fields).filter(k => allowed.includes(k)) },
      ip_address: req.ip,
    });
    ...
    res.status(500).json({ error: '운행 수정에 실패했습니다.' });
  }
});
